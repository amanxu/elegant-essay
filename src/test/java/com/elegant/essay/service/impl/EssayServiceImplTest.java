...
import com.elegant.essay.mapper.EssayCategoryMapper;
...
public class EssayServiceImplTest {
    ...
    @Test
    public void createEssay() throws Exception {
        ...
        // 文章内容
        String essayContent = essayBuf.toString();
        Integer essayLength = essayContent.length();
        Integer bound = essayLength - 80;
        for (int i = 0; i < 2000000; i++) {
            EssayDto essayDto = new EssayDto();
            essayDto.setCategoryId(categories.get(random.nextInt(categorySize)).getId());
            essayDto.setAllowComment(random.nextInt(10) % 2);
            essayDto.setUserId(userVos.get(random.nextInt(userSize)).getUserId());
            essayDto.setPublishTime(new Date());
            if (random.nextInt(10) % 2 == 0) {
                essayDto.setStatus(EssayStatusEnum.PUBLISHED.getCode());
            } else {
                essayDto.setStatus(EssayStatusEnum.DRAFT.getCode());
            }
            essayDto.setSort(random.nextInt(5));
            int titleBound = random.nextInt(bound);
            essayDto.setTitle(essayContent.substring(titleBound, titleBound + 10));
            int summaryBound = random.nextInt(bound);
            essayDto.setSummary(essayContent.substring(summaryBound, summaryBound + 20));
            int contentBound = random.nextInt(bound);
            essayDto.setContent(essayContent.substring(contentBound, contentBound + 50));
            essayDto.setImgUrl("https://elegant-essay.oss-cn-beijing.aliyuncs.com/f1fa8595cc134c1a8481109743c48697.png");
            essayService.createEssay(essayDto);
        }
    }

    private static StringBuilder essayBuf = new StringBuilder("１９７５年二、三月间，一个平平常常的日子，细蒙蒙的雨丝夹着一星半点的雪花，正纷纷淋淋地向大地飘洒着。时令已快到惊蛰，雪当然再不会存留，往往还没等落地，就已经消失得无踪无影了。黄土高原严寒而漫长的冬天看来就要过去，但那真正温暖的春天还远远地没有到来。" +
